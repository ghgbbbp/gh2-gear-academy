...
fn handle_start_game() {
    ...
    match &session.status {
        SessionStatus::StartGameWaiting | SessionStatus::CheckWordWaiting => {
            // Send start game request to Wordle program
        ...
        // If reply received, handle game start event
...
fn handle_check_word(word: String) {
    ...
    match &session.status {
        SessionStatus::CheckWordWaiting => {
            ...
            // If exceeded max check count or timeout, game is lost
        ...
        // If status is incorrect, throw error
